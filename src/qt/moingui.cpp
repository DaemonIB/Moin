...
MoinGUI::MoinGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new MoinBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("Moin") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/moin"));
    setWindowIcon(QIcon(":icons/moin"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/moin/index.html");
#else
    QFile html("/opt/moin/index.html");
#endif
...
void MoinGUI::pageLoaded(bool ok)
{
...
void MoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("Moin") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/moin_testnet"));
            setWindowIcon(QIcon(":icons/moin_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/moin_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Moin client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/moin_testnet"));
...
void MoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void MoinGUI::setMessageModel(MessageModel *messageModel)
{
...
void MoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Moin client"));
    trayIcon->setIcon(QIcon(":/icons/moin"));
...
#ifndef Q_OS_MAC
void MoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MoinGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("title", tr("%n active connection(s) to Moin network", "", count));
}
...
void MoinGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void MoinGUI::changeEvent(QEvent *e)
{
...
void MoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void MoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::MOIN, nFeeRequired));
...
void MoinGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void MoinGUI::gotoSignMessageTab(QString addr)
{
...
void MoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void MoinGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void MoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MoinGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void MoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Moin address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void MoinGUI::handleURI(QString strURI)
{
...
void MoinGUI::setEncryptionStatus(int status)
{
...
void MoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void MoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void MoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MoinGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void MoinGUI::updateStakingIcon()
{

...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog, MoinBridge *bridge)
{
    if (!model) return;
    ...
    } else
    {
        ...
        int nDisplayUnit = BitcoinUnits::MOIN;
        ...
        l8->setStyleSheet((nChange > 0 && nChange < CENT) ? "color:red;" : ""); // Change < 0.01MOIN

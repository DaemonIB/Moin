...
#include "moinbridge.moc"

MoinBridge::MoinBridge(MoinGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
// This is just a hook, we won't really be setting the model...
void MoinBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void MoinBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void MoinBridge::copy(QString text)
{
...
// Options
void MoinBridge::populateOptions()
{
...
// Transactions
bool MoinBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void MoinBridge::clearRecipients()
{
    recipients.clear();
}

bool MoinBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 MOIN, 1 MOINX
        switch(rcp.txnTypeInd)
        {
            case TXT_MOIN_TO_MOIN:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MOIN, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_MOIN_TO_ANON:
                formatted.append(tr("<b>%1</b> to MOINX %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MOIN, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MOIN, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsMoinXBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your MOINX balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MOIN, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void MoinBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void MoinBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap MoinBridge::listAnonOutputs()
{
...
void MoinBridge::populateTransactionTable()
{
...
void MoinBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void MoinBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString MoinBridge::transactionDetails(QString txid)
{
...
// Addresses
void MoinBridge::populateAddressTable()
{
...
void MoinBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void MoinBridge::updateAddressLabel(QString address, QString label)
{
...
bool MoinBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool MoinBridge::deleteAddress(QString address)
{
...
void MoinBridge::populateMessageTable()
{
...
void MoinBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool MoinBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool MoinBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString MoinBridge::getPubKey(QString address)
{
...
QString MoinBridge::createGroupChat(QString label)
{
...
QString MoinBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList MoinBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
// Blocks
QVariantMap MoinBridge::listLatestBlocks()
{
...
QVariantMap MoinBridge::findBlock(QString searchID)
{
...
QVariantMap MoinBridge::blockDetails(QString blkHash)
{
...
QVariantMap MoinBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap MoinBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap MoinBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap MoinBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap MoinBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap MoinBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Moin prefix.");
            return result;
        }
...
QVariantMap MoinBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap MoinBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap MoinBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
